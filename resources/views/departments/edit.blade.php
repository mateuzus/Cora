...
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>Edição de departamento - {{$departments->name}}</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('departments.index') }}">Departamentos</a></li>
                <li class="breadcrumb-item active">Edição de departamento - {{$departments->name}}</li>
            </ol>
        </div>
    </div>
@endsection
            ...
            <div class="col-12">
                <div class="card">
                    {{ Form::model($departments, [
                            'url'=>route("departments.update", $departments->id),
                             'files'=>true,
                             'method'=>"put"
                     ]) }}
                    <div class="card-header">
                        <h3 class="card-title">Edição de Departamento</h3>
                    </div>
                    <div class="card-body">
