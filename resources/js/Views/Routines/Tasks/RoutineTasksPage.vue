                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title"><strong>Listagem de tarefas do POP:</strong> {{ this.routine.name }}
                                </h3>
...
export default {
    ...
    methods: {
        getData() {
            this.isLoading = true;
            try {
                let url = `/api/v1/routinetasks?routine=${this.routine.id}`;
        ...
        storeTask(){
            ...
            let url = `/api/v1/routinetasks?pop=${this.routine.id}`;
        ...
        updateTask(task){
            ...
            this.isLoading = true;
            let url = `/api/v1/routinetasks/${task.id}?pop=${this.routine.id}`;
            axios.put(url, task)
                .then(response=>{
                    let timerInterval
                    Swal.fire({
        ...
        deleteTask(index, task){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let url = `/api/v1/routinetasks/${task.id}`;
